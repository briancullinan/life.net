...
namespace ExpressionSerialization
{
	...
	public partial class ExpressionSerializer
	{
		...
		internal XElement ConstantExpressionToXElement(ConstantExpression e)
		{
            ...
            var attr = value.GetType().GetCustomAttributes(true).Select(x => x.GetType());
		    if (attr.Contains(typeof (CompilerGeneratedAttribute)))
		    {
                var field = value.GetType().GetFields().First();
		        ...
		        XElementValues[1] = GenerateXmlFromProperty(typeof (System.Linq.Expressions.ExpressionType),
		                                                    "NodeType", value ?? string.Empty);
                value = field.GetValue(((ConstantExpression)e).Value);
		...
		internal XElement MemberExpressionToXElement(MemberExpression e)
		{
		    ...
		    var attrs = ((MemberInfo) value).DeclaringType.GetCustomAttributes(true).Select(x => x.GetType());
		    if (attrs.Contains(typeof (CompilerGeneratedAttribute)))
		    {
                return GenerateXmlFromExpressionCore(((MemberExpression)e).Expression);
