...
namespace Files.Activities
{
    public class AuditFiles : Activity
    {
        ...
        {
            get { return GetValue<AccessMask>(); }
            set { SetValue(value); }
        }
        ...
        private void ProcessEntry(EventLogEntry entry, ActivityQueue queue)
        {
            const int MAX = 0;
                            ...
                            var log = new Filesystem
                                {
                                    ...
                                    AccessMask = decimal.Parse(newData["mode"]),
                                    Success = true,
