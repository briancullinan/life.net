...
namespace iLife.Utilities.Shaders
{
    using System;
    ...
    public class SmoothMagnifyEffect : ShaderEffect
    {
        ...
        static SmoothMagnifyEffect()
        {
            ...
            var resourceStrings = assembly.GetManifestResourceNames();

            foreach (var name in resourceStrings)
            {
                if (!name.Contains("SmoothMagnify.ps")) continue;
                var embeddedStream = assembly.GetManifestResourceStream(name);
                pixelShader.SetStreamSource(embeddedStream);
                break;
            }
        }
