    ...
    public sealed class Message : ISerializable
    {
        ...
        private IEnumerable<MailAddress> GetMailAddresses(string header)
        {
            ...
            foreach (var address in matches)
            {
                try
                {
                    var display = address.Value;
                    if (display.Contains(address.Groups[1].Value))
                        display = address.Value.Replace(address.Groups[1].Value, "").Trim('\t', ' ', '"', '<', '>', '\r', '\n').Trim();
                    mailAddresses.Add(new MailAddress(address.Groups[1].Value, display));
                }
                catch (Exception ex)
                {
        ...
        public DateTime GetDate()
        {
            ...
            if (value == null)
            {
                foreach (var rx in RxDates)
                {
                    var match = rx.Matches(_message.FirstOrDefault(x => x.StartsWith("Date:")) ?? "")
                      .Cast<Match>().LastOrDefault();
                    if (match == null) continue;
                    value = match.Value.ToNullDate();
                    if (value != null)
